...
from .models import Game, GameState, SystemState, Vendor, Event
...
def game_state_view(request, game_state_id):
    ...
    events = Event.objects.filter(game=this_game.game, at_day__gte=this_game.days_left).order_by("at_day")

    return render(request, 'game/game_state.html', {'game_state': this_game, 'events': events})
